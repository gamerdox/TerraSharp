...
from backend.app.models.schemas import GridCellRisk, RiskRasterSummary, ContributingFactors
...
def get_point_risk(lat: float = Query(...), lon: float = Query(...)):
    ...
    if not (grid.min_lon <= lon <= grid.adjusted_max_lon and grid.adjusted_min_lat <= lat <= grid.max_lat):
        raise HTTPException(
            status_code=400,
            detail=f"Coordinate ({lat}, {lon}) is outside current AOI bounds: {list(grid.bounds)}",
        )
